...
/**
 * What a visitor did on a creator's public profile. Written by the beacon
 * endpoint (src/app/api/analytics/route.ts), read only by staff.
 *
 ...
 *                     is hidden or closed; one row per visible stretch
 */
...
/**
 * One row per thing a visitor did on a creator's public profile. Append-only:
 * nothing updates a row after insert, and the staff dashboard reads them back
 * as aggregates (src/lib/data/analytics.ts).
 *
 * `creator_id` rather than `profile_id`, deliberately: every other table here
 * belongs to the profile it points at, whereas an event is ABOUT a creator but
 * generated by someone else. The name keeps that distinction visible in every
 * query. Cascades with the profile, so deleting a creator takes their history.
 *
 * `viewer_session_id` is whoever generated the event: the signed-in viewer's
 * auth user id when there is one, otherwise a random id the browser keeps in
 * localStorage. Enough to count distinct viewers and to say "this ticket was
 * used once per person"; NOT an identity, and the raw IP is never stored —
 * the same line ask_message draws.
 *
 * `metadata` carries whatever the event type needs, and a label snapshot
 * (promo code, link label) so the dashboard still has something to show after
 * the row it pointed at is deleted:
 *   promo_click / promo_used  { promoId, code }
 *   link_click                { kind, href, label }
...
export const analyticsEvent = pgTable("analytics_events", {
  ...
  creatorId: uuid("creator_id")
    .notNull()
    .references(() => profile.id, { onDelete: "cascade" }),
  viewerSessionId: uuid("viewer_session_id"),
  ...
  creatorTypeCreatedIdx: index("analytics_events_creator_type_created_idx").on(
    table.creatorId,
    table.eventType,
    table.createdAt,
  ),
...
export const analyticsEventRelations = relations(analyticsEvent, ({ one }) => ({
  creator: one(profile, {
    fields: [analyticsEvent.creatorId],
    references: [profile.id],
  }),
}));
